...
const double min_depth = 10;
const double min_median_depth = 10;
const double min_initial_depth = 10;
const double max_initial_frequency = 0.05;
...
inline bool filter_timepoint(Timepoint const & timepoint){
    if((timepoint.median_depth < min_median_depth) || (timepoint.depth < min_depth)){
        return true;
    }
    else{
        return false;
...
inline bool passes_filter(Trajectory const & trajectory){
    ...
    if(initial_frequency > max_initial_frequency){
        return false;
    }
    
    double initial_two_frequency = trajectory[0].alt/trajectory[0].depth + trajectory[1].alt/trajectory[1].depth;
    if(initial_two_frequency>0.2){
        return false;
    }
    ...
    int num_good_timepoints = 0; // # timepoints w/ >= 2 alts
    ...
    for(int i=1,imax=trajectory.size();i<imax;++i){
        ...
        if(!filter_timepoint(timepoint)){
            ...
            if(timepoint.alt >= 5 && frequency >= 0.05){
    ...
    if(num_good_timepoints < 2){
        return false;
    }
    
    if(num_confirmed_timepoints < 2){
        return false;
    }
...
inline double calculate_relaxation_time(Trajectory const & trajectory, bool is_bootstrap=false){
    ...
    for(int i=0,imax=trajectory.size();i<imax;++i){
        ...
        if( (i>0) and (running_avg_f/running_avg_weight > avg_f*0.50) ){
            relaxation_time = i;
            break;
        }
