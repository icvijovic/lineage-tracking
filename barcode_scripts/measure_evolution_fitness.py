...
while it < max_iterations:
	...
	for this_clone_index in range(0,len(clone_dict.keys())):
		if sum(mask[this_clone_index]) > 0:
			...
			expectation_grid = np.einsum('ijk,j->ijk', expectation_grid, evolution_total_counts)
			...
			llhs = np.einsum('ijk,j->ijk',llhs,1./evolution_kappas)
			llhs += 0.75* numpy.log(clone_counts_after[:,:,np.newaxis])
			llhs += -0.25* numpy.log(expectation_grid) 
			llhs += 0.5 * numpy.log(4*numpy.pi*evolution_kappas[np.newaxis,:,np.newaxis])
